...
`text-davinci-003` and `llama2` are examples of LLMs.
...
`gpt-4`, `gpt-3.5-turbo`, and `claude-2` are examples of chat models.
...
You can use `PromptTemplates` with chat models as well, but for more sophisticated prompts you can use a `MessagePromptTemplate`. You can build a `ChatPromptTemplate` from one or more `MessagePromptTemplate`s. You can use `ChatPromptTemplate`'s `format_messages` method to generate the formatted messages.

Because this is generating a list of messages, it is slightly more complex than the normal prompt template, which only generates a string. Please see the sections on prompts to understand more options available to you here.
...
Now that we've got a model and a prompt template, we'll want to combine the two. Many LangChain modules, including LLMs, chat models, and prompts,
can be chained together using LangChain Expression Language to form sequences where the output of one step is passed as the input of the next step.
...
A simple and common example of a chain passes an input first to a PromptTemplate and then to an LLM. We can construct a chain from the above model and prompt template like this:
...
You can create chains using chat models and `ChatPromptTemplate`s as well:

<ChainChatModel/>

Unlike LLMs, chat models return chat messages. To standardize the output as a string, you'll notice we've added an output parser as a third step in the chain.
This particular output parser takes the output messages from the chat model and extracts the content, and returns it as a string.
